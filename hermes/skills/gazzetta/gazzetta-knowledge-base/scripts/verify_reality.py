...
def main():
    ...
    # 2. JSON freshness (< 15 min old = FRESH)
    ...
    if gen_at:
        try:
            ...
            if age_min >= 60:
                failures.append(f"JSON frozen: {age_min:.0f} min old — pipeline may be dead")
            print(f"  generated_at: {gen_at}")
            story_count = len(public_json.get("stories", [])) + 1
            print(f"  stories: {story_count}")
            lead = public_json.get("lead", {})
            print(f"  lead: {lead.get('headline','?')[:80]}")
        except Exception as e:
            print(f"  ERROR parsing date: {e}")
            failures.append("Cannot parse generated_at from public JSON")
    ...
    if os.path.exists(db_path):
        ...
        cur = db.execute("SELECT COUNT(*) FROM stories")
    ...
    try:
        ...
        if "LAGGING" in public_html or "DIVERGENT" in public_html:
            ...
            print("  Trade hooks: divergence labels NOT found")
            failures.append("Trade hooks missing divergence labels")
    except:
        print("  Trade hooks: could not verify (network error)")
        pass
    ...
    # 3. DB drafts piling up?
    db_path = os.path.join(PROJECT_DIR, "gazzetta.db")
    if os.path.exists(db_path):
        db = sqlite3.connect(db_path)
        cur = db.execute("SELECT COUNT(*) FROM drafts WHERE status='pending'")
        pending = cur.fetchone()[0]
        if pending > 100:
            print(f"  WARNING: {pending} pending drafts — approve_draft.py may not be running")
            failures.append(f"{pending} pending drafts accumulating")
        else:
