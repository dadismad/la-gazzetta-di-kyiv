...
// Escape regex special characters in a keyword string
function escapeRegex(s) {
  return s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}

// Word-boundary match: keyword must appear as a whole word or phrase
...
function matchesSector(setup) {
  ...
  return KEYWORDS.some(k => wordMatch(all, k));
}
...
async function boot() {
  ...
  const filteredStories = siteStories.filter(s =>
    ...
    KEYWORDS.some(k => wordMatch((s.headline + s.thesis).toLowerCase(), k))
  );
