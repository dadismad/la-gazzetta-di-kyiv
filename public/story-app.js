...
// v24.0: +multi-persona blocks (C-Suite/Quant/Degen)
...
(function() {
  ...
  function renderMultiPersona(story) {
    ...
    h += '<h2 class="intel-section-label">' + t('multi_persona_label', 'MULTI-PERSONA ANALYSIS') + '</h2>';
    h += '<div class="persona-grid">';
    if (mp.c_suite) {
      var cs = mp.c_suite;
      h += '<div class="persona-card persona-csuite">';
      h += '<div class="persona-header"><span class="persona-icon">&#x1F3DB;</span><span class="persona-role">' + t('persona_csuite', 'C-SUITE') + '</span></div>';
      h += '<div class="persona-headline">' + (cs.headline || '') + '</div>';
      h += '<div class="persona-body">' + (cs.body || '') + '</div>';
      if (cs.implication) h += '<div class="persona-implication"><span class="persona-imp-label">' + t('persona_implication', 'Board-level') + ':</span> ' + cs.implication + '</div>';
      h += '</div>';
    }
    if (mp.quant) {
      var q = mp.quant;
      h += '<div class="persona-card persona-quant">';
      h += '<div class="persona-header"><span class="persona-icon">&#x1F4CA;</span><span class="persona-role">' + t('persona_quant', 'QUANTITATIVE') + '</span></div>';
      h += '<div class="persona-headline">' + (q.headline || '') + '</div>';
      h += '<div class="persona-body">' + (q.body || '') + '</div>';
      if (q.metrics) {
        h += '<div class="persona-metrics">';
        if (q.metrics.flow_direction) h += '<span class="persona-metric">' + q.metrics.flow_direction + '</span>';
        if (q.metrics.asset_class) h += '<span class="persona-metric asset-tag">' + q.metrics.asset_class.toUpperCase() + '</span>';
        h += '</div>';
      }
      h += '</div>';
    }
    if (mp.degen) {
      var dg = mp.degen;
      h += '<div class="persona-card persona-degen">';
      h += '<div class="persona-header"><span class="persona-icon">&#x1F3AF;</span><span class="persona-role">' + t('persona_degen', 'EXECUTION') + '</span></div>';
      h += '<div class="persona-headline">' + (dg.headline || '') + '</div>';
      h += '<div class="persona-body">' + (dg.body || '') + '</div>';
      if (dg.signal) {
        var sig = dg.signal;
        h += '<div class="persona-signal">';
        h += '<span class="persona-sig-dir ' + (sig.direction || '').toLowerCase() + '">' + (sig.direction || '') + '</span>';
        if (sig.conviction) h += '<span class="persona-sig-conviction">' + sig.conviction + '</span>';
        if (sig.entry_zone && sig.entry_zone !== 'TBD' && sig.entry_zone !== 'awaiting review') h += '<span class="persona-sig-entry">Entry: ' + sig.entry_zone + '</span>';
        h += '</div>';
      }
      h += '</div>';
    }
...
async function init() {
    ...
    // Load stories (English-only)
    const dataPath = STORIES_PATH;
